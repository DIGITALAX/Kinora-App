...
const Stages: FunctionComponent<StagesProps> = ({
  ...
  return (
    <div
      className={`relative w-full p-px flex rounded-md border border-white lg:top-20 ${
        questStage !== QuestStage.Post
          ? "h-full lg:h-5/6 lg:max-w-[20rem]"
          : "h-96 lg:max-w-[30rem]"
      }`}
    >
      <div className="relative w-full flex flex-col rounded-md justify-between bg-nave h-full">
        <div className="relative w-full p-2 flex items-center justify-center text-white font-bit bg-nave border-b border-white">
          {questStage}
        </div>
        {questStage !== QuestStage.Post ? (
          <div
                          ...
                          (item?.toLowerCase() == "gates" &&
                            (questInfo?.details?.gated?.erc20Addresses?.length >
                              ...
                              questInfo?.details?.gated?.erc721TokenIds
                                ?.length > 0)) ? (
                            <MdOutlineCheckBox color="FBDB86" size={20} />
                          ) : (
                            <MdOutlineCheckBoxOutlineBlank
                              color="FBDB86"
                              size={20}
        ...
        <div className="relative w-full p-2 h-fit flex items-center justify-center text-white bg-nave border-t border-white font-bit mb-0 flex-row gap-1">
          {questStage !== QuestStage.Details && (
            <div
