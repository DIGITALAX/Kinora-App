...
const PostQuote: FunctionComponent<PostQuoteProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`relative w-full h-full p-2 flex items-center justify-start flex-col cursor-pointer to-black bg-gradient-to-r rounded-md gap-5`}
        onClick={(e) => {
          e.stopPropagation();
          quote?.openActionModules?.[0]?.contract?.address
            ?.toLowerCase()
            ?.includes(KINORA_OPEN_ACTION?.toLowerCase())
            ? router.push(`/quest/${quote?.id}`)
            : {};
        }}
      >
