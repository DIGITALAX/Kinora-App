...
const Gifs: FunctionComponent<{ dict: any }> = ({ dict }): JSX.Element => {
  const contexto = useContext(ModalContext);

  const { manejarGif, buscarGifs, setBuscarGifs, gifCargando } = useGifs();
        ...
        <div className="relative w-full h-full flex flex-col gap-3 p-2 items-center justify-start">
          <div className="relative w-fit h-fit items-end justify-end ml-auto cursor-pointer flex">
            <ImCross
              color="#FBDB86"
              size={10}
              onClick={() => contexto?.setGif({ open: false })}
            />
          </div>
          <div
          ...
          >
            ...
            <div className="relative flex items-start justify-center overflow-y-scroll w-full h-full">
              <div className="flex flex-wrap items-start justify-center gap-3 w-fit h-fit">
                {buscarGifs.gifs?.map((gif: any, index: number) => {
                  return (
                    <div
                      key={index}
                      onClick={() => {
